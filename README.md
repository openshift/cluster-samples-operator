...
- An openshift-samples secret (if supplied)
...
-- An 'openshift-samples' secret has been placed in the operator’s namespace and that secret has been copied into the openshift namespace
- ConfigurationValid
...
Deletion of the samples resource will reset the samples operator to the default configuration, but leave the current revision of the sample operator pod running.
